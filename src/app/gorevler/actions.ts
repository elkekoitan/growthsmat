...
import { createTask, deleteTask, toggleTaskDone, assignTask, updateTaskLog, type RealTask, type TaskInput } from "@/server/repositories/tasks";
...
/** Saha günlüğü (not/ölçüm) yazar — görev tamamlandığında gerçek bir kayıt bırakmak için. */
export async function updateTaskLogAction(id: string, note: string, measurement: string): Promise<RealTask | undefined> {
  const { membership } = await requireMembership();
  const task = await updateTaskLog(id, membership.workspaceId, { note, measurement });
  revalidatePath("/gorevler");
  return task;
}
